...
* Bug fix (all models): `ReportingMetrics.t_stat` uses `stdtrit` directly with baseline `ddof` rather than going through `t_stat()` which treats its argument as a sample size and subtracts 1 internally (matches the daily model's `optimize_results.py` convention of `DoF + 1`).
