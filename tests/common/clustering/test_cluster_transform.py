...
from opendsm.common.clustering.transform.fpca import (
    _fourier_coefficient_map,
    _fourier_design,
    _fpca_base,
    _fpca_explained_variance,
    _fpca_max_components,
    _fpca_spectrum,
...
class TestFourierBasis:
    ...
    def test_basis_is_orthonormal_in_l2(self, n_basis):
        ...
        design = _fourier_design(x, n_basis)
...
class TestFpcaRegression:
    ...
    def test_explained_variance_matches_recorded_spectrum(self, loadshape):
        ...
        actual = _fpca_explained_variance(loadshape, np.arange(loadshape.shape[1]), 6)
    ...
    def test_transform_matches_recorded_scores(self, loadshape):
        ...
        scores = _fpca_transform_with_n(np.arange(loadshape.shape[1]), loadshape, 4)
    ...
    def test_parallel_analysis_selects_recorded_component_count(self, loadshape):
        ...
        n_max = _fpca_max_components(*loadshape.shape)
        spectrum = partial(
            _fpca_spectrum,
            coefficient_map=_fourier_coefficient_map(x, n_max + 4),
            n_max=n_max,
        )
        n = _parallel_analysis_n_components(loadshape, spectrum, seed=0)
    ...
    def test_transform_is_reproducible(self, loadshape):
        ...
        x = np.arange(loadshape.shape[1])
        first = _fpca_transform_with_n(x, loadshape, 4)
        second = _fpca_transform_with_n(x, loadshape, 4)
