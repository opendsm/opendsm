...
def _difference_matrices(
    ...
    def base_difference_matrix(n):
        # First-order difference: [-1, 1] on diagonal bands
        return np.diff(np.eye(n), n=1, axis=0)

    o = bspline_degree

    D = [None, None, None]  # D1, D2, D3
    eye_cache = None  # cache identity matrix for fallback branch
    for i in range(3):
        if o > i:
            ...
            # singular matrix. The corresponding rows become standard
            ...
            fd = base_difference_matrix(n_base_funcs - i)
            _D = np.linalg.solve(W, fd)  # inv(W) @ fd

            D[i] = _D if i == 0 else _D @ D[i-1]

        else:
            # Spline order <= derivative order: use standard difference
            if eye_cache is None or eye_cache.shape[0] != n_base_funcs:
                eye_cache = np.eye(n_base_funcs)
                
            D[i] = np.diff(eye_cache, n=i+1, axis=0)
...
def _derivative_zones(
    ...
    For the i-th first-difference row, the representative position is the
    midpoint of its knot span [knots[i+1], knots[i+k+1]].  Zone membership
    is determined by comparing that midpoint to the breakpoints.
    ...
    # Midpoint of knot span [knots[i+1], knots[i+k+1]] for each derivative interval
    positions = 0.5 * (knots[1:1 + n_deriv] + knots[k + 1:k + 1 + n_deriv])

    knot_idx = {
        "hdd": np.flatnonzero(positions < bp[0]),
        "tidd": np.flatnonzero((positions >= bp[0]) & (positions <= bp[1])),
        "cdd": np.flatnonzero(positions > bp[1]),
    }
...
def fit_daily_pspline(
    ...
    if bc_type == "clamped" and bspline_degree >= 1:
...
class DailyPSpline(BSpline):
    ...
    def _set_bp(
        ...
        """Optimize breakpoint positions via bounded scalar minimization.

        Iterates over bp knots, adjusting each position to minimize penalized
        wRMSE. The penalty pushes breakpoints toward data bounds to maximize
        TIDD width. After each sweep, regenerates weights via adaptive loss
        and checks convergence (rtol=1e-3).
        ...
        weights : ndarray or None
            Data weights.
        alpha : float or "adaptive"
            Alpha for adaptive_weights.
        max_sweeps : int
            Maximum optimization sweeps.
        xatol : float
            Absolute tolerance for minimize_scalar.
        ...
        """
        knots = np.asarray(internal_knots, dtype=float).copy()
        bp = np.asarray(bp, dtype=float).copy()

        # Find bp indices in knots
        bp_idx = []
        for b in bp:
            idx = np.argmin(np.abs(knots - b))
            if np.isclose(knots[idx], b):
                bp_idx.append(idx)
        ...
        for sweep in range(max_sweeps):
            for j, i in enumerate(bp_idx):
                if i < 1 or i >= len(knots) - 1:
                    continue
                ...
                def objective(new_pos):
                    trial_knots = knots.copy()
                    trial_knots[i] = new_pos
                    trial_bp = bp.copy()
                    trial_bp[j] = new_pos

                    # Remove all knots between trial_bp[0] and trial_bp[1]
                    mask = (trial_knots < trial_bp[0]) | (trial_knots > trial_bp[1])
                    # Always keep the boundary knots
                    mask[0] = True
                    mask[-1] = True
                    ...
                    if weights is not None:
                        wrmse = np.sqrt(np.average(residuals ** 2, weights=weights))
                    else:
                        wrmse = np.sqrt(np.mean(residuals ** 2))

                    # BP penalty: push bp toward data bounds
                    bp_dist = np.array([trial_bp[0] - x[0], x[-1] - trial_bp[1]])
                    bp_dist *= wrmse / x_range

                    if lasso_a != 0:
                        wrmse += lasso_a * np.linalg.norm(bp_dist, 1)
                    if ridge_a != 0:
                        wrmse += ridge_a * np.linalg.norm(bp_dist, 2)
            ...
            # End-of-sweep: fit for convergence check
            trial_knots = knots.copy()
            mask = (trial_knots < bp[0]) | (trial_knots > bp[1])
            mask[0] = True
            mask[-1] = True
            ...
            spl = BSpline(t=padded, c=coefs, k=self.bspline_degree, extrapolate=True)
            residuals = (y - spl(x)).reshape(-1)

            if weights is not None:
                current_wrmse = np.sqrt(np.average(residuals ** 2, weights=weights))
            else:
                current_wrmse = np.sqrt(np.mean(residuals ** 2))

            # Regenerate weights using adaptive loss
            a_weights, _, weight_alpha = adaptive_weights(
                residuals, alpha=alpha, C_algo="mad",
            )
            ...
            if weight_alpha != 2.0:
                if weights is None:
                    ...
                    weights = weights * a_weights

                weights = _rescale_to_range(weights, new_min=1.0, new_max=100.0)

            print(f"BP Sweep {sweep + 1}, wRMSE: {current_wrmse:.6f}, weight_alpha: {weight_alpha:.4f}")

            if np.isclose(current_wrmse, prior_wrmse, rtol=1e-3):
                break

            prior_wrmse = current_wrmse
        ...
        # If there are fewer than n_min data points between breakpoints, merge them as average
        n_tidd = x.size - n_hdd - n_cdd
    ...
    def fit(self, x, y, bp=None, weights=None, zone_knot_count=None) -> "DailyPSpline":
        ...
        # Initial bp guess from smoothing spline minimum, or user-provided
        if bp is not None:
            bp_standardized = (np.asarray(bp, dtype=float) - self.x_mean) / self.x_std
        else:
            y_vals = self.knots.spl(self.knots.spl_x)
            x_min = self.knots.spl_x[np.argmin(y_vals)]
            bp_standardized = np.array([x_min, x_min])
        ...
        for n in range(n_iter):
            ...
            bp_standardized = self._set_bp(
                ...
                bp_standardized, internal_knots,
                weights=self.weights,
            )

            # Re-place knots with updated breakpoints
            internal_knots = self.knots.get_internal_knots(
                bp=bp_standardized,
                n_knots=zone_knot_count,
                n_min=self.n_min,
