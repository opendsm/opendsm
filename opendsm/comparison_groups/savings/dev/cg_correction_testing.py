...
class Savings:
    ...
    def _agg_cluster_data(self):
        ...
        if self.reject_outliers:
            ...
            df_cluster = pd.concat(
                [
                    df_cp[["cluster", "datetime", "ls_key"]]
                    .groupby(["cluster", "datetime"])
                    .first(),
                    df_cp[["cluster", "datetime", "temperature"]]
                    .groupby(["cluster", "datetime"])
                    .median(),
                ],
                axis=1,
        ...
        cols_scaled = [
            col.replace("_scale", "") for col in df_cluster.columns if col.endswith("_scale")
        ]
    ...
    def _get_treatment_cg_data(self):
        ...
        cluster_data = np.array(
            df_cluster.set_index(["cluster", g])[self.df_cols]
    ...
    def add_abs_pct_did(self, simplified_eqn=False):
        ...
        if simplified_eqn:
            ...
            res[~match] = (2 - df["ratio_cg"][~match]) * df["modeled_t"][~match] - df["observed_t"][
                ~match
            ]
    ...
    def add_sig_pct_did(self, k=0.01, m_0=0.1):
        ...
        scale = (
            (df["modeled_t"] - df["observed_t"]) * sigmoid(np.abs(df["modeled_t"]), m_0, k)
            + df["observed_t"]
        ) / (
            (df["modeled_cg"] - df["observed_cg"]) * sigmoid(np.abs(df["modeled_cg"]), m_0, k)
            + df["observed_cg"]
        )
    ...
    def add_scaled_ordinary_did(self):
        ...
        df_cg_scale = (
            df_cg_scale.unstack().reset_index().rename(columns={"level_0": "ls_key", 0: "scale"})
        )
    ...
    def add_modeled_scaled_ordinary_did(self):
        ...
        df_ratio = df_ratio.rename(
            columns={"temperature_cg": "temperature", "diff_ratio": "observed"}
        )

        ratio_modeled = []
        for id in df_ratio["id"].unique():
            df_ratio_id = df_ratio[df_ratio["id"] == id]
            df_ratio_id_baseline = df_ratio_id[df_ratio_id["period"] == "baseline"][
    ...
    def df(self):
        ...
        df_t_cg[did_cols] = df_t_cg[did_cols].where(
            ~df_t_cg[measured_cols].isna().any(axis=1), np.nan
        )
