...
class StratifiedSamplingModel:
    ...
    See `StratifiedSamplingModel.add_column()` for more information on configuring columns.
    Once columns are added, execute the model as follows:

        m.fit_and_sample(df_treatment, df_pool)

    See `StratifiedSamplingModel.fit_and_sample()` for additional options, notably several
